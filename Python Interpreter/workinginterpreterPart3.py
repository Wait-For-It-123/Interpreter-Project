...
def interpreter(input, output):
    ...
    answer = []
    # This keeps track of functions. The key is the name of the function and each key contains the "list" of instructions for the function and the argument the function takes
    fun_Dict = {}
    # This keeps track of when the function itself preforms the bind opperation
    fun_bindDict = {}
    # This is so I can keep track of items that are bound before the function stats going through its instructions
    boundB4Fun = {}
    # This keeps the the original function call instructions and argument so when it gets called again it can go back to the original and recompute
    nestedFunCall_dict = {}
    # Keep track of what items need to be deleted within the let for functions to work properly
    toDelete = []
    # To keep track of what the function needs to bind to, only used for inOutFun
    argToBindToFunOutput = []
    # Keep track of when nested let has multiple same arguments
    ...
    # Make a counter to work with the to delete list for let and functions to work properly
    toDeleteCounter = 0
    # Counter to be able to change let bind dict when going through nested lets
    mlbv = 0
    # Counter to keep track of how many variables have the same value in nested lets
    multipleLetBoundVarCount = 0
    # A Check to see if inOutFun is in the works
    is_inoutfun = False
    # A check to see if return is in the set of function instructions, if it's not then add return into function instructions so it can return the correct value
    returnInFun = False
    # A check to see if the multiple let bound items dictionary is empty or not
    multipleLetBoundVarDictIsEmpty = True
    # Checks to see if there is a nested function call
    nestedFunCall = False
...
#interpreter("input_3.txt", "output.txt")
